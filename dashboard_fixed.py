...
# Load data suhu rata-rata per kota per tahun (format wide)
...
if page == "📊 Data":
    ...
    st.dataframe(city_data.style.format({col: "{:.2f}" for col in city_data.select_dtypes(include='number').columns}))

    st.markdown("---")
    st.subheader("📅 Rata-rata Suhu per Kota per Tahun")

    # Ubah kolom tahun menjadi string
    av_suhu_kota_pertahun.columns = av_suhu_kota_pertahun.columns.map(str)
    selected_year_str = str(selected_year)

    if selected_year_str in av_suhu_kota_pertahun.columns:
        tampil_df = av_suhu_kota_pertahun[["City", selected_year_str]].copy()
        tampil_df.rename(columns={selected_year_str: "TEMP"}, inplace=True)
        tampil_df["TEMP"] = pd.to_numeric(tampil_df["TEMP"], errors="coerce")
        st.dataframe(tampil_df.style.format({"TEMP": "{:.2f}"}))
    else:
        st.warning(f"Tahun {selected_year} tidak tersedia dalam data.")
...
# Rata-rata Suhu per Kota
    ...
    fig2, axes = plt.subplots(4, 3, figsize=(20, 10), sharex=True)
    for i, city in enumerate(df_filtered["City"].unique()):
        ax = axes.flatten()[i]
        hourly = df_filtered[df_filtered["City"] == city].groupby("hour")[pollutants].mean()
        hourly.plot(ax=ax)
        ax.set_title(city)
        ax.set_xlabel("Jam")
